...
| `xps-skeleton` | Loading placeholder: `currentColor` at low opacity with a pulse animation, suppressed under `prefers-reduced-motion`. One modifier per part it stands in for — `--title` (a result title), `--text` (a body line), `--block` (a media square), `--heading` (a facet/tree/range heading), `--box` (a checkbox square), `--count` (a facet count, pushed to the end of its row), `--control` (the text inside a select or number input; the box around it is the widget's own control element), `--track` (the range rail) and `--line` (a whole sentence or a control label). Each modifier carries the design's width through `--xps-skeleton-width`; restate that property in a stylesheet rule to vary one bar's width (that is how the skeleton blocks below get their per-row widths — never an inline `style`). |
...
`xps-suggestions--skeleton` — which `shell.css` uses to vary the bar widths per row (the markup
carries no `style` attribute, so a strict `style-src` Content Security Policy costs nothing), and
which a site can hang its own skeleton design on.
...
| `xps-suggestions__group` | `<li role="group" aria-labelledby>` | One per source, in order: recent searches (client-side), query suggestions, matching documents. Omit the group wrapper when there is only one ungrouped source and put `role="option"` on `<li>` directly. Two things bring the wrapper back for a single source: the recents, whose group is always labelled because the label row carries the Clear control, and a response whose entries carry `Suggestion.group` — the server sends it in every suggest mode, so a lone Pages group still says **Pages**, in the standalone widget and in the search box's popup alike. |
