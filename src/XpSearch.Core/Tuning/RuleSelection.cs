...
{
    /// <summary>
    /// Selects the rules that apply: enabled, inside their schedule and matching the query, ordered by
    /// <see cref="TuningRule.Priority"/> ascending, then by <see cref="TuningRule.Id"/> ascending.
    /// </summary>
    ...
    public static IReadOnlyList<TuningRule> Active(IEnumerable<TuningRule> rules, string query, DateTime utcNow)
    {
        ...
        [
            .. rules
                .Where(rule => rule.Enabled && InSchedule(rule, utcNow) && Matches(rule, query ?? string.Empty))
    ...
    /// <returns>An entry of the form <c>rule:&lt;name&gt;</c>.</returns>
    public static string Explain(TuningRule rule)
    {
        ArgumentNullException.ThrowIfNull(rule);

        return string.Create(CultureInfo.InvariantCulture, $"rule:{rule.Name}");
    }
}
