...
templates, and the markup each one emits. Every widget is a connector plus a default renderer — the
...
<script>
  const { searchBox, refinementList, stats, hits, pagination } = xpsearch;
  ...
  search.addWidgets([
    ...
    refinementList({ container: '#facet-content-type', attribute: 'contentType', label: 'Content type' }),
...
```js
import xpsearch, { searchBox, hits, refinementList, pagination, stats, sortBy } from '@yourco/xperience-search';
```
...
See the [JavaScript client guide](js-client.md) for `xpsearch()` options, routing and the event bus,
...
highlight for that field, it falls back to the plain field value, escaped. Ask for the fields you
want highlighted in the instance options:

```js
xpsearch({ index: 'site-content', highlight: { fields: ['title', 'content'], snippetLength: 160 } });
```
...
searchBox({
  ...
  showSubmit: false,
  autofocus: false,
  queryHook: (query, refine) => refine(query.trim()),
});
```
...
| `queryHook` | — | `(query, refine) => void`. Nothing reaches the state unless you call `refine`. |
...
icon buttons carry an `aria-label` with their glyph `aria-hidden`. Typing refines (debounced by the
...
|---|---|---|
| `container` | — | Selector or element. Required. |
| `templates.item` | title link + highlighted snippet + content-type meta, and an image when the hit has one | `(hit, helpers) => Renderable` |
| `templates.empty` | "No results for …" | `({ query }, helpers) => Renderable` |
| `templates.loading` | `loadingRows` skeleton rows | `(helpers) => Renderable` |
| `transformItems` | — | `(hits) => hits`, applied before rendering. |
| `loadingRows` | `3` | Skeleton rows in the default loading template. |

Markup: `<div class="xps xps-hits">` containing the live region `xps-hits__status` and either
`xps-hits__list` (an `<ol>` of `xps-hits__item`) or `xps-hits__empty`. Modifiers `--empty` and
`--loading` (with `aria-busy="true"`) mirror the state.

Accessibility: `xps-hits__status` is a `role="status"` element (`aria-live="polite"` by implication)
that is **created once and only has its text replaced**, so a re-render that does not change the
count is not announced twice. `stats` renders the same number without a live region, so a page with
both announces once. The default item template emits a real heading and a real link.

Click tracking (spec §9.1): the widget delegates `click` on its root, and any `<a>` inside a result
sends `{ eventType: 'click', objectID, position, queryId }` to `/api/xpsearch/events`.
...
| `sortBy` | `['isRefined', 'count:desc', 'name:asc']` | Applied left to right. |
| `transformItems` | — | `(items) => items`, after sorting and capping. |

Markup: `<div class="xps xps-refinement-list">` (plus `--searchable`) with `__title`, the optional
...
the hit area and no `for`/`id` pairing is needed. The show-more button carries `aria-expanded` and is
`disabled` rather than removed, so focus survives. A value that drops to count 0 renders `disabled`
...
| `totalPages` | — | Caps `nbPages` for indexes where deep paging is unwanted. |
...
Accessibility: every enabled control is a real `<a href>` whose target is `createURL(page)` — the
same URL routing would produce — so results pages are crawlable and open in a new tab; a plain left
click is intercepted and refines instead. A disabled end control is a `<span aria-disabled="true">`
...
stats({
  ...
  templates: {
    text: (data, { html, formatNumber }) => html`${formatNumber(data.nbHits)} matches`,
  },
});
```
...
|---|---|---|
| `container` | — | Selector or element. Required. |
| `templates.text` | `46 results in 14 ms` | `(data, helpers) => Renderable`. `data` is the stats render state: `nbHits`, `processingTimeMS`, `query`, `page`, `nbPages`, `hitsPerPage`, `hasResults`. |
| `emptyText` | `'Type to search.'` | Shown before the first response. |

Markup: `<div class="xps xps-stats">` (plus `--empty`) containing `xps-stats__text`, and in the
default template `xps-stats__time` around the timing. **Not** a live region — `hits` owns the
announcement.
...
|---|---|---|
| `container` | — | Selector or element. Required. |
| `items` | — | `{ label, value }[]`. `value` is `relevance` or an index-configured sort key. Required. |
| `label` | `'Sort by'` | Label text. |
| `hideLabel` | `false` | Adds `xps-sr-only` to the label; it stays associated. |

Markup: `<div class="xps xps-sort-by">` with `xps-sort-by__label` (`<label for>`) and
`xps-sort-by__select` (a native `<select name="sort">`). The select is built once and only its
`value` is patched, so changing the sort does not destroy the element you are using.

## `clearRefinements`

```js
clearRefinements({ container: '#search-clear', label: 'Clear filters' });
```
...
| `includedAttributes` / `excludedAttributes` | — | Which refinements count towards "is there anything to clear". |

Markup: `<div class="xps xps-clear-refinements">` (plus `--disabled`) with
`xps-clear-refinements__button`. The button is `disabled` — never removed — when nothing is refined,
so pressing it repeatedly does not throw focus to the body. It clears facet *and* numeric
refinements; it does not clear the query.
...
| `includedAttributes` / `excludedAttributes` | — | Which refinements to show. |
| `transformItems` | — | `(items) => items`. |

Markup: `<div class="xps xps-current-refinements">` (plus `--empty`) with an `xps-sr-only` `__title`,
`__list` and one `__item` per refinement, each holding an `xps-chip` with `xps-chip__attribute`,
...
rather than removed, so the layout does not jump. Numeric refinements read as `price <= 50`.

## `toggleRefinement`

```js
toggleRefinement({ container: '#toggle-english', attribute: 'language', value: 'en', label: 'English only' });
```
...
`_highlights` is safe to insert. Client-side, that is the *only* markup the widgets trust
automatically:
...
| `highlight(field, hit)` | Trusted markup (the server's `_highlights[field]`), else the escaped plain field |
...
`autocomplete`, `rangeSlider`, `hierarchicalMenu` and `infiniteHits` have markup contracts and (for
range) a connector, but no default renderer yet. `connectRange` is public today — see
[Building a custom widget](custom-widgets.md) — and the remaining connectors follow the open
