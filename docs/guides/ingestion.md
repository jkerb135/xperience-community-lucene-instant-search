...
the last five minutes. That failure is recorded in the ingestion log (as an `index` operation), which
is why every instance of a web farm reports the same health — and why an index that has recovered
