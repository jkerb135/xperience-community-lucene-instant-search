...
| Pagination | Style (numbered pages / load more button) — "load more" emits a `loadMore` mount instead of a `pagination` one, so place one or the other, never both. It steps by the search's page size and cannot go past the index's *Maximum result window* · Pages either side of the current one (3 by default) · Show the "first page" control · Show the "last page" control — the last three appear for numbered pages only |
...
| Active filters | Screen-reader heading · Keep the chips on one scrolling row · Attribute names (one `attribute;Label` per line) — only needed for an attribute whose own filtering widget is not on the page, since a chip otherwise reads that widget's label. The chips row of the design: one removable chip per refinement. Place a Clear filters widget beside it for the trailing "Clear all" |
...
The facet list and the category tree render their title as a button with a chevron, and pressing it
folds the group away. That is the shipped default: the sidebar of the design shows a chevron on
every group. Untick **Title folds the group** for a group that must always stay open.

The fold is a view state only. It is not saved, it is not in the page URL, and it changes nothing
about the search — a folded group keeps its selected values, and they keep showing in the Active
filters chips.

#### The chips row and the two clear-all buttons

**Search - Active filters** and **Search - Clear filters** are separate widgets so an editor can put
them where the design wants them:

- in the filter column's heading — a "Filters" heading of yours plus one Clear filters widget;
