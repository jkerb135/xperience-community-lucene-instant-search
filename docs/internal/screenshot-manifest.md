...
| `tuning--status.png` | `…/edit/2/status` | built index (35 entries) | `src/XpSearch.Admin/UIPages/IndexStatus.cs`, `Client/src/status/IndexStatusTemplate.tsx` + `.module.scss` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only; UX-3b then rebuilt the page to `docs/internal/design/Status.dc.html` — a header card, four figure tiles, the stacked bar and the two tables; RB-1 then made the rebuild state durable: the header line reports the last finished rebuild, the fourth tile reads **Rebuild started**, and a stuck rebuild adds a **Rebuild may have failed** tag and callout) |
| `tuning--analytics.png` | `…/edit/2/analytics` | query-log rows (HW-11 seeding) | `src/XpSearch.Admin/UIPages/Analytics/AnalyticsDashboardPage.cs`, `Client/src/analytics/AnalyticsDashboardTemplate.tsx`, `AnalyticsDashboard.module.scss` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only) |
| `tuning--query-tester.png` | `…/edit/2/query-tester` (runs "coffee") | indexed content | `src/XpSearch.Admin/UIPages/QueryTester/QueryTesterPage.cs`, `Client/src/query-tester/QueryTesterTemplate.tsx` | 2026-09-01 — **STALE** (QT-2 rebuilt the page as one diff table with a verdict callout, the pipeline trail and a row detail panel; QT-3a restyled it to the approved design) |
| `tuning--query-tester-row.png` | `…/edit/2/query-tester` (run "espresso", click the boosted row) | demo tuning rules (QT-2c host seeding) | `src/XpSearch.Admin/UIPages/QueryTester/QueryTesterPage.cs`, `Client/src/query-tester/QueryTesterTemplate.tsx` | **PENDING** — new in QT-2; the row detail panel with the score steps, restyled in QT-3a. Add the shot to `routes.json` and capture with the rest |
| `tuning--rules.png` | `…/edit/2/rules` | seeded rule | `src/XpSearch.Admin/UIPages/Rules.cs` | 2026-09-01 |
| `tuning--rule-builder.png` | `…/edit/2/rules` → open seeded rule | seeded rule | `src/XpSearch.Admin/UIPages/RuleBuilder/RuleBuilderPage.cs`, `Client/src/rule-builder/RuleBuilderTemplate.tsx` + `.module.scss` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only; UX-3b then rebuilt the page to `docs/internal/design/RuleBuilder.dc.html` — one header card holding the rule's settings, then the If and Then flows) |
| `tuning--rule-condition-panel.png` | `…/edit/2/rules` → open seeded rule → **Edit** a condition | seeded rule | `Client/src/rule-builder/ConditionPanel.tsx`, `AttributeRows.tsx`, `RuleBuilderTemplate.module.scss` | **PENDING** — new in UX-4; the condition panel rebuilt to `docs/internal/design/rule-builder-panels/Main.dc.html` (the three switch sections, their field rows and the attribute rows). Add the shot to `routes.json` and capture with the rest |
| `tuning--rule-action-panel.png` | `…/edit/2/rules` → open seeded rule → **Edit** a *Pin an item* action | seeded rule with a pin action | `Client/src/rule-builder/ActionPanel.tsx`, `ItemPicker.tsx`, `RuleBuilderTemplate.module.scss` | **PENDING** — new in UX-4; the pin action panel rebuilt to `docs/internal/design/rule-builder-panels/ActionPanel.dc.html` (the item picker over Position). Add the shot to `routes.json` and capture with the rest |
...
| `experiments--detail.png` | `…/edit/2/experiments` → open experiment | concluded experiment | `src/XpSearch.Admin/UIPages/Experiments/ExperimentPages.cs`, `Client/src/experiments/ExperimentDetailTemplate.tsx` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only) |
...
| Image | URL / route | Data prerequisites | Source files (staleness triggers) | Captured |
|---|---|---|---|---|
| `themes--kentico-orange.png` | `themes/test/section-kentico-orange.html` (opens from disk, no server) | — | `themes/src/scss/tokens/_kentico-orange.scss`, `themes/src/kentico-orange.css`, `themes/fixtures/*.html` | **STALE** — new in TH-8, never captured, and TH-9 added `fixtures/sidebar.html` and changed the suggestions/search-box empty state, so both section pages moved. Shoot it beside `section-default.html` so the guide's *Two shipped palettes* section can show the pair. TH-12 also changed `fixtures/active-filters.html` (the chip now splits key/value), and SK-1 added `fixtures/skeleton.html` — a nineteenth section on both pages. |
| `themes--kentico-orange.png` | `themes/test/section-kentico-orange.html` (opens from disk, no server) | — | `themes/src/scss/tokens/_kentico-orange.scss`, `themes/src/kentico-orange.css`, `themes/fixtures/*.html` | **STALE** — new in TH-8, never captured; TH-9 added `fixtures/sidebar.html` and changed the suggestions/search-box empty state, TH-10 rewrote every theme selector, and TH-11 added the single-source autocomplete variant and restored the panel's row spacing — so both section pages moved. Shoot it beside `section-default.html` so the guide's *Two shipped palettes* section can show the pair. |
