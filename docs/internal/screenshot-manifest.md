...
| `tuning--status.png` | `…/edit/2/status` | built index (35 entries) | `src/XpSearch.Admin/UIPages/IndexStatus.cs`, `Client/src/status/IndexStatusTemplate.tsx` + `.module.css` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only) |
...
| `tuning--rule-builder.png` | `…/edit/2/rules` → open seeded rule | seeded rule | `src/XpSearch.Admin/UIPages/RuleBuilder/RuleBuilderPage.cs`, `Client/src/rule-builder/RuleBuilderTemplate.tsx` + `.module.scss` | 2026-09-01 — **STALE** (the custom pages were put on one layout rhythm: 24px between cards, no page-level padding, tokens only) |
