...
| `tuning--query-tester.png` | `…/edit/2/query-tester` (runs "coffee") | indexed content | `src/XpSearch.Admin/UIPages/QueryTester/QueryTesterPage.cs`, `Client/src/query-tester/QueryTesterTemplate.tsx` | 2026-09-01 — **STALE** (QT-2 rebuilt the page as one diff table with a verdict callout, the pipeline trail and a row detail panel) |
| `tuning--query-tester-row.png` | `…/edit/2/query-tester` (run "espresso", click the boosted row) | demo tuning rules (QT-2c host seeding) | `src/XpSearch.Admin/UIPages/QueryTester/QueryTesterPage.cs`, `Client/src/query-tester/QueryTesterTemplate.tsx` | **PENDING** — new in QT-2; the row detail panel with the score steps. Add the shot to `routes.json` and capture with the rest |
